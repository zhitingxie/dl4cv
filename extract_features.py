...
# grab the list of images that we'll be describing then randomly shuffle them to allow for easy training 
...
# initialize the HDF5 data writer, then store the class label names in the dataset
...
for i in np.arange(0, len(imagePaths), bs):
    # extract the batch of images and labels, then initialize the list of actual images that will be passed
    # through the network for feature extraction
    batchPaths = imagePaths[i:i+bs]
    batchLabels = labels[i:i+bs]
    batchImages = []

    # loop over the image and labels in the current batch
